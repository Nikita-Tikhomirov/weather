...
Сессии проектного чата:
- при входе APK подключается к последней сессии проекта;
- bridge отдает replay последних сообщений из session log;
- кнопка `Новая сессия` создает чистый session log для этого проекта;
- кнопка `Остановить DeepSeek` отправляет команду остановки текущего процесса;
- session log хранится в `<project>\.deepseek\state\sessions\<project_id>\`.
