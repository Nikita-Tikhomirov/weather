...
Старые сетки без суффикса не удаляются и остаются в `source_grids`/`library`.
Это тоже наш сгенерированный набор, поэтому в приложение импортируются оба
каталога: `library_v2` и `library`. Новый `manifest.json` считает готовыми
только файлы из `source_grids_v2` и `library_v2`, а `library` остается
первым сгенерированным набором для загрузки в каталог.
...
Она импортирует `library_v2` и `library`, заполняет `chat_stickers.asset_url`
ссылками через `CHAT_MEDIA_DISK` и деактивирует только стикеры, которых нет в
этих сгенерированных наборах. Файлы из первого набора `library` получают
префикс `gen1_` в `sticker_id`, чтобы одноименные v2-файлы не перезаписывались.
`pack_key` формируется как `group_style_category`, например
`rats_plush_3d_emotions` или
...
По умолчанию утилита ожидает сетку 5x5 и использует объектную резку: убирает
chroma-key фон `#00ff00`, находит связные компоненты на всей сетке и назначает
их ближайшей ячейке. Так в PNG попадает весь свой стикер, даже если хвост или
аксессуар залез за границу ячейки, а части соседних стикеров отбрасываются.
После этого утилита триммит пустые края и кладет каждый стикер на квадратный
холст 512x512.
Для второй сетки в той же категории используйте `--start-index 26`, для третьей -
`--start-index 51` и так далее.

Пересобрать оба уже сгенерированных набора без новой генерации:

```powershell
python .\scripts\stickers\reslice_sticker_libraries.py
```
